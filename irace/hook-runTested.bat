...
rem -- java -d64 -Xmx5g -cp "../target/*:../target/lib/*" br.ufpr.inf.gres.irace.core.HookRun %FIXED_PARAMS% %CAND_PARAMS% > %LOGS% 2>&1
rem --               CURRENT  ;  CLASSES     ;      LIBS      ;        MATH3
rem -- java -cp ".;..\..\target\classes;..\..\target\lib;C:\Users\Pc\.m2\repository\org\apache\commons\commons-math3\3.3\commons-math3-3.3.jar:org.apache.commons.collections.list.SetUniqueList" br.ufpr.inf.gres.irace.core.HookRun %FIXED_PARAMS%%CAND_PARAMS% > %LOGS% 2>&1
java -jar ../iraceFinal.jar %FIXED_PARAMS%%CAND_PARAMS% > %LOGS% 2>&1


for /f "delims=" %%l in (FUN_%CANDIDATE%) do set fitnessValue=%%l
echo %fitnessValue%
